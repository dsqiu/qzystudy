...
// are adjusted in 5 degree increment by a keyboard callback.

static int shoulderAngle = 0,elbowAngle = 0;

// Handles the keyboard envent: the left and right arrows bend the elbow, the
...
// the glScaledf call does not pollute code that follows a call to myWireBox.
...
// Display the arm in its current position and orientation. The whole
// function is bracketed by glPushMatrix and glPopMatrix calls because every
// time we call it we are in an "evironment" in which a glutLookAt is in
// effect. (Note that in particular,replacing glPushMatrix with
// glLoadIndentity makes you lose the camera setting from glutLookAt).
...
// the thing about glutWriteBox is that normally its origin is in the middle
...
// relativce to the rotation that we already made above to orient the upper
// arm. So, we want to rotation elbow defrees about the z-axis. But,like
// before, the anchor point for the rotating is at the end of the box, so
...
// whole visible region, then sets the prohection matrix to something reason-
...
// does application initialization; enters the main envent loop.
